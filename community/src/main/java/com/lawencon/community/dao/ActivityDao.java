...
public class ActivityDao extends AbstractJpaDao {
	...
	@SuppressWarnings("unchecked")
	public List<PojoResportIncomesAdminResData> getActivityIncome(Float percentIncome, LocalDate startDate,
			LocalDate endDate, String typeCode) {
		final List<PojoResportIncomesAdminResData> resultList = new ArrayList<>();
		BigDecimal percentValue = new BigDecimal(Float.toString(percentIncome));
		final StringBuilder sqlQuery = new StringBuilder();
		...
		for (Object objs : result) {
			final Object[] obj = (Object[]) objs;
			final PojoResportIncomesAdminResData data = new PojoResportIncomesAdminResData();
